...
// ImprovedSignupRequest represents the JSON request body for signup
type ImprovedSignupRequest struct {
	Email         string `json:"email"`
	Password      string `json:"password"`
	FallbackEmail string `json:"fallback_email"`
}

// ImprovedSignupResponse represents the JSON response for signup
type ImprovedSignupResponse struct {
	Message string `json:"message"`
	Status  string `json:"status"`
}

// improvedSignupHandlerFactory returns an HTTP handler for user signup that stores pending signups
// instead of creating users immediately. This ensures fallback email verification before user creation.
func improvedSignupHandlerFactory(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Check if email already exists in users table (FIRST - most specific check)
		...
		// Check if email has a pending signup (SECOND - specific to this email)
		...
		// Check user count limit (THIRD - general system limit)
		var userCount int
		...
		// Create pending signup instead of creating user immediately
