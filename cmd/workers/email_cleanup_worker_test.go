...
// MockR2Client implements R2ClientInterface for testing
...
func (m *MockR2Client) GetEmailMetadata(ctx context.Context, blobID string) (map[string]string, error) {
	return map[string]string{"content-type": "application/octet-stream"}, nil
}
...
func TestEmailCleanupWorker_ExpiredEmailDeletion(t *testing.T) {
	...
	// Create worker with mock R2 client
	mockR2Client := &MockR2Client{}
	...
	expiredTime := time.Now().UTC().Add(-1 * time.Hour) // 1 hour ago (UTC)
...
func TestEmailCleanupWorker_NoDeletionForValidEmails(t *testing.T) {
	...
	futureTime := time.Now().UTC().Add(1 * time.Hour) // 1 hour in future (UTC)
	insertTestEmail(t, worker.db, "valid-email-1", "blob-3", &futureTime, false, 0)
...
func insertTestEmail(t *testing.T, db *sql.DB, emailID, blobID string, expiresAt *time.Time, burnAfterRead bool, accessCount int) {
	...
	if expiresAt != nil {
		// Use UTC format to match the query
