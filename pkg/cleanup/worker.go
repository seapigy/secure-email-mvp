...
// NewEmailCleanupWorkerWithDB creates a new cleanup worker using an existing database connection
func NewEmailCleanupWorkerWithDB(db *sql.DB, cleanupIntervalMinutes int) (*EmailCleanupWorker, error) {
	// Initialize R2 client
	r2Client, err := storage.NewR2ClientFromEnv()
	if err != nil {
		return nil, fmt.Errorf("failed to create R2 client: %w", err)
...
func (w *EmailCleanupWorker) getEmailsToDelete() ([]EmailToDelete, error) {
	query := `
		...
		FROM emails 
		WHERE (
			(expires_at IS NOT NULL AND expires_at <= datetime('now', 'utc')) OR
	...
	for rows.Next() {
		...
		if email.ExpiresAt != nil && time.Now().After(*email.ExpiresAt) {
...
func (w *EmailCleanupWorker) GetCleanupStats() (map[string]interface{}, error) {
	...
	err := w.db.QueryRow(`
		...
		WHERE expires_at IS NOT NULL AND expires_at <= datetime('now', 'utc') AND encrypted_blob_url IS NOT NULL
	`).Scan(&expiredCount)
	if err != nil {
