 ...
 * 
 * THIS FILE CONTAINS THE PLAN SELECTOR COMPONENT.
 * 
 * 🚨 CRITICAL RULES:
...
const PlanSelector: React.FC<PlanSelectorProps> = ({ accountType, onSelect, selectedPlan, onBack }) => {
  const getPlans = (): Plan[] => {
    if (accountType === 'paid') {
      return [
        {
  ...
  const handleSelect = (planId: string) => {
    log.info('Plan selected', { planId, accountType }, 'PlanSelector');
    onSelect(planId);
  };
      ...
      <div className="bg-white dark:bg-secondary-900 rounded-2xl shadow-xl border border-secondary-200 dark:border-secondary-700 p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-secondary-600 dark:text-secondary-400">
            Select the plan that best fits your {accountType === 'company' ? 'organization' : 'needs'}
          </p>
        </div>
        ...
        <div className="mt-6">
          <button
            type="button"
            onClick={onBack}
            className="w-full px-4 py-2 text-sm font-medium text-secondary-700 dark:text-secondary-300 bg-white dark:bg-secondary-800 border border-secondary-300 dark:border-secondary-600 rounded-lg hover:bg-secondary-50 dark:hover:bg-secondary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors"
          >
            Back
          </button>
        </div>
